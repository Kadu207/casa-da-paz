...
|------:|--------|---------------|------|
| **65025** | Baixo | SSH host | Porta SSH atual; `:22` não escuta |
| 80 / 443 | Baixo | nginx host | — |
| **5440** | **OK-filtrado** | `inova-gastro-360-postgres` | publish `0.0.0.0:5440` + harden |
...
| **20243/tcp** | **OK-local** | `excellence-cloudflared-n8n` (host network) | `--metrics 127.0.0.1:20243` |

### Swarm / cloudflared — notas

- Cluster Swarm: **1 manager / 1 node** (`inovati-server`).
...
HTTPS: `https://casadapaz.inovatitech.com.br/health` → 200.
...
### Allowlist SSH (opcional — porta 65025)

Hoje `ACCEPT tcp dpt:65025` é aberto a qualquer origem. Para restringir ao IP do admin:

```bash
# No PC: curl.exe -4 -s https://ifconfig.me
MY_IP=177.206.120.41   # substitua pelo IPv4 atual do PC

# Manter sessão SSH aberta e testar em OUTRA janela antes de fechar
sudo iptables -I INPUT 3 -p tcp -s ${MY_IP}/32 --dport 65025 -j ACCEPT
sudo iptables -D INPUT -p tcp --dport 65025 -j ACCEPT
sudo iptables -A INPUT -p tcp --dport 65025 -j DROP
sudo sh -c 'iptables-save > /etc/iptables/rules.v4'
```

**Risco:** IP dinâmico muda → perde SSH. Tenha console Hetzner como fallback.
